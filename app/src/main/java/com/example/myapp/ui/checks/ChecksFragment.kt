...
import com.example.myapp.databinding.FragmentChecksBinding
...
class ChecksFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val checksViewModel =
            ViewModelProvider(this).get(ChecksViewModel::class.java)

        _binding = FragmentChecksBinding.inflate(inflater, container, false)
        val root: View = binding.root

        val textView: TextView = binding.textNotifications
        checksViewModel.text.observe(viewLifecycleOwner) {
            textView.text = it
        }
        return root
    }
